...
class LinkfaceDataset(imdb):
  # data_set: VehicleLicense, VehicleText, VOC2007
  # image_set: trainval, test
  # classes: ('__background__',  'text')
  def __init__(self, data_set, image_set, classes, devkit_path=None):
    imdb.__init__(self,  data_set + '_' + image_set)
    self._image_set = image_set
    self._data_set = data_set
    self._devkit_path = self._get_default_path() if devkit_path is None \
      else devkit_path
    self._data_path = os.path.join(self._devkit_path, data_set)
    ...
    assert os.path.exists(self._devkit_path), \
      '{} path does not exist: {}'.format(data_set, self._devkit_path)
  ...
  def _load_image_set_index(self):
    ...
    with open(image_set_file) as f:
      for x in f.readlines():
        x = x.strip()
        ann_file = os.path.join(self._data_path, 'Annotations', x + '.xml')
  ...
  def _load_pascal_annotation(self, index):
    ...
    non_others_objs = [
     obj for obj in objs if obj.find('name').text != 'others']
    objs = non_others_objs
    ...
    gt_quadbox = np.zeros((num_objs, 8), dtype=np.int32)
    overlaps = np.zeros((num_objs, self.num_classes), dtype=np.float32)
    # "Seg" area for pascal is just the box area
    seg_areas = np.zeros((num_objs), dtype=np.float32)
    recognize = np.zeros((num_objs), dtype=np.float32)

    # Load object bounding boxes into a data frame.
    for ix, obj in enumerate(objs):
      bbox = obj.find('bndbox')
      if bbox.find('xmin') == None:
        continue
      if obj.find('recognize') != None:
        recognize[ix] = float(obj.find('recognize').text)
      else:
        recognize[ix] = float(-1.0)
      # print(recognize[ix])
  ...
  def _write_voc_results_file(self, all_boxes):
    for cls_ind, cls in enumerate(self.classes):
      if cls == '__background__':
        continue
      print('Writing {} {} results file'.format(cls, self._data_set))
  ...
  def _do_python_eval(self, output_dir='output'):
    ...
    for i, cls in enumerate(self._classes):
      ...
      aps += [ap]
      print(('REC  for {} = {:.4f}'.format(cls, rec[-1])))
      print(('PREC for {} = {:.4f}'.format(cls, prec[-1])))
...
if __name__ == '__main__':
  ...
  classes = ('__background__',  'text')
  d = LinkfaceDataset('VehicleLicense', 'trainval', classes)
  res = d.roidb
